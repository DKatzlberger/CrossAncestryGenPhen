...
We apply a permutation-based aggregation approach to assess the stability and significance of test statistics computed independently for each feature (e.g., gene) across resampling iterations. 
The perm_aggregate_sets() function aggregates statistics per feature using a user-defined function (such as the mean) and compares the observed value to a null distribution generated by permuting the values within each feature across iterations. 
This method is particularly sensitive to features that show consistently strong and stable effects across resampling, effectively down-weighting noisy or unstable signals. It accounts for the dependence structure induced by repeated estimation and provides empirical p-values without relying on parametric assumptions. 
As a result, Type I error is conservatively controlled, while reduced power (Type II error) may occur for features with weak or highly variable signals.

## Test statistics aggregation
The function `perm_aggregate_sets()` is able to aggregate test_statistics or p-values. 
Here we use it in a two-sided manner to aggregate the test statistics across iterations.
```{r aggregation_test_statistics}
agg_res <- perm_aggregate_sets(
  x = perm_combined,
  T_col = "T_obs",
  feature_col = "feature",
  agg_fun = mean,
...
The output contains summary statisitcs under `$summary_stats` and the null distribution in `$T_perm`
```{r summary_stats}
head(agg_res$summary_stats, 10)
```

The null distribution can be plotted using the `plot_T_distribution()` function.
```{r T_perm_distribution, fig.alt = "Null distribution of aggregated permuted test statistics"}
plot_T_distribution(
  x = agg_res, 
  statistic = "T_perm",
